...
#define HID_KEYBOARD_IN_RPT_LEN              USBD_REPORT_BYTE_KB
...
static uint8_t advertData[] = {
    ...
    'f', 'e', 'r', 'c', 'u', 's'  // connection interval range
};

// Device name attribute value
static const uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "fercus";
...
void HidEmu_deinit(void)
{
    uint8_t initial_advertising = FALSE;
...
//            usbd_kb_report_send(kb_rep_send);
            HidDev_Report(HID_RPT_ID_KEY_IN, HID_REPORT_TYPE_INPUT,
                          HID_KEYBOARD_IN_RPT_LEN, (uint8_t *) kb_report_usbd);
//            printf("rep\n");
        }
        tmos_set_event(hidEmuTaskId, START_REPORT_EVT);

//        tmos_start_task(hidEmuTaskId, START_REPORT_EVT, MS1_TO_SYSTEM_TIME(1));
