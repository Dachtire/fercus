...
//    HAL_LedInit();
    led_gpio_init();
    led_tim_init(144, 1, 0, TIM2);
    led_tim_init(144, 1, 0, TIM3);
#endif
#if(defined HAL_KEY) && (HAL_KEY == TRUE)
...
//    kb_fn_exti_init();
    ...
    kb_col_gpio_init();
    kb_row_gpio_init();
//    kb_row_gpio_adc_init();
//    kb_adc_dma_init();

    eta9184_vbus_gpio_init();
//    eta9184_vbus_exti_init();
    eta9184_gpio_init();
//    eta9184_pulse_it_init();
//    eta9184_pulse_tim_init();
//    eta9184_dischrg_exti_init();
//    eta9184_adc_init();
//    eta9184_adc_get();
...
uint8_t OnBoard_SendMsg(uint8_t registeredTaskID, uint8_t event, uint8_t state, void *data)
{
  ...
  if ( registeredTaskID != TASK_NO_TASK )
  {
    ...
    if ( msgPtr )
    {
      ...
      msgPtr->pData = data;
      tmos_msg_send( registeredTaskID, ( uint8_t * ) msgPtr );
