...
uint8_t USBFSTaskID = INVALID_TASK_ID;
...
uint16_t USBFS_ProcessEvent(uint8_t task_id, uint16_t events)
{
    ...
    if (events & SYS_EVENT_MSG) {
        ...
        if ((pMsg = tmos_msg_receive(USBFSTaskID)) != NULL) {
            USB_ProcessTMOSMsg((tmos_event_hdr_t *) pMsg);
            // Release the TMOS message
            tmos_msg_deallocate(pMsg);
        }
    ...
    if(events & USB_IN_EVT){
        ...
        //        usbd_kb_report_send(kb_report_usbd);
        keyboard_usb_send();
        tmos_start_task(USBFSTaskID, USB_IN_EVT, MS1_TO_SYSTEM_TIME(1));
...
//        //#endif
//
//        return (events ^ USB_HOST_INIT_EVT);
//    }
    return 0;
}
...
{
    USBFSTaskID = TMOS_ProcessEventRegister(USBFS_ProcessEvent);

    tmos_start_task(USBFSTaskID, USB_INIT_EVT, MS1_TO_SYSTEM_TIME(1000));
    tmos_start_task(USBFSTaskID, USB_IN_EVT, MS1_TO_SYSTEM_TIME(2000));
}